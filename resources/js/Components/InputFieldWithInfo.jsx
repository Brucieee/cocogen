    ...
    return (
        <div className={`w-[214px] group relative flex flex-col transition-all duration-300 ${isInvalid ? 'group-hover:bg-status-light-red' : ''}`}>
            ...
            <div className="relative flex items-center">
                {/* Show icon only if input is empty or valid */}
                {(!hasInput || isCocogen) && (
                    <img src={'/icons/InfoIcon.svg'} alt="Info Icon" className="absolute right-2 h-5 w-5" />
                )}
                <input
                    type="text"
                    ...
                    className={`h-[44px] w-[214px] border-0 border-b-2 bg-transparent flex items-center pb-[10px] gap-2.5 p-[10px] text-[14px] font-normal outline-none transition-all duration-300 focus:ring-0 focus:outline-none
                        ${isInvalid ? 'border-red-600 text-red bg-status-highlight-red hover:bg-status-light-red' : 'border-secondary text-gray-500 group-hover:bg-teal-1 group-hover:border-teal-9 group-hover:placeholder-black'}`}
                />
            </div>
        </div>
    );
